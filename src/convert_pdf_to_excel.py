...
def main():
    ...
    for yr in [2023, 2024, 2025]:
        pdf_path = BASE_DIR / f'Penjualan Detail {yr}.pdf'
        print(f"Membaca {pdf_path.name}...")
        df = parse_pdf(pdf_path, known_mapping)
        n_trx = df['No Transaksi'].nunique() if not df.empty else 0
        print(f"  -> Tahun {yr}: {len(df)} baris item, {n_trx} transaksi")
        dfs.append(df)
    ...
    out_xlsx = BASE_DIR / 'dataset 2023-2025.xlsx'
    out_excel = BASE_DIR / 'dataset 2023-2025.excel'

    print(f"\nMenyimpan ke {out_xlsx.name}...")
    with pd.ExcelWriter(out_xlsx, engine='openpyxl') as writer:
        df_all.to_excel(writer, sheet_name='Item Detail', index=False)

    shutil.copy(out_xlsx, out_excel)

    print(f"Selesai! Berhasil membuat dataset gabungan dengan total {len(df_all)} baris.")
